                            ...
                            {isLoadingNotifications ? (
                                ...
                                <ul className="space-y-4">
                                    {notifications.map((notification) => {
                                      ...
                                      return (
                                      <li key={`notif-${notification.id}`} className={`p-4 rounded-md border flex flex-col sm:flex-row justify-between items-start gap-4 transition-colors ${isUnread ? 'bg-primary/5 border-primary/20' : 'bg-card'}`}>
                                          <div className="flex-1 space-y-1">
                                              <div className="flex items-center justify-between flex-wrap gap-x-2">
                                                   <span className={`text-xs font-semibold uppercase tracking-wider ${isUnread ? 'text-primary' : 'text-muted-foreground'}`}>
                                                      {notification.type.replace(/_/g, ' ')} {/* Replace underscores */}
                                                  </span>
                                                   <Badge variant={isUnread ? 'default' : 'outline'} className="capitalize ml-auto">
                                                      {notification.status}
                                                   </Badge>
                                              </div>
                                              <p className={`font-semibold text-secondary-foreground ${isUnread ? 'font-bold': ''}`}>{notification.title}</p>
                                              <p className="text-sm text-muted-foreground">{notification.message}</p>
                                              <p className="text-xs text-muted-foreground/80 pt-1">{formatTimestamp(notification.createdAt)}</p>
                                          </div>
                                          <div className="flex-shrink-0 flex flex-col sm:flex-row gap-2 items-end sm:items-center w-full sm:w-auto pt-2 sm:pt-0">
                                               {/* Button to View Details or Mark as Read */}
                                                <Button
                                                    variant={relatedLink ? "outline" : (isUnread ? "secondary" : "ghost")}
                                                    size="sm"
                                                    onClick={() => handleNotificationLinkClick(relatedLink, notification.id)}
                                                    disabled={isUpdating && isUnread} // Disable only if updating this specific unread item
                                                    className={`w-full sm:w-auto ${!isUnread && !relatedLink ? 'cursor-default opacity-50' : ''}`} // Style disabled-like if read and no link
                                                >
                                                    {isUpdating && isUnread ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin"/> :
                                                     relatedLink ? <ExternalLink className="mr-1.5 h-3.5 w-3.5" /> :
                                                     isUnread ? <Check className="mr-1.5 h-3.5 w-3.5" /> :
                                                     <Check className="mr-1.5 h-3.5 w-3.5 opacity-50" /> // Dim checkmark if read and no link
                                                     }
                                                    {relatedLink ? 'View Details' : (isUnread ? 'Mark as Read' : 'Read')}
                                                </Button>
                                          </div>
                                      </li>
                                    )})}
